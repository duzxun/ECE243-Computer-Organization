...
RESET_SS: 
	MOV R0, #0
	CMP	R10, #0x3b //check if reached 59
	BGE	RESET_DD
	ADD R10, #1		//increment
	B	START_STOP
...
DISPLAY:    LDR     R8, =0xFF200020 // base address of HEX3-HEX0
			...
			MOV     R3, R10         // display R10 on HEX3-2
            BL      DIVIDE          // ones digit will be in R0; tens
                                    // digit in R1
			
			MOV     R9, R1          // save the tens digit
            BL      SEG7_CODE 
			LSL     R3, #16         //left shift 16 bits
			ORR     R6, R3          //HEX2

            MOV     R3, R9          // retrieve the tens digit, get bit code
            BL      SEG7_CODE       
            LSL     R3, #24			//left shift 24 bits
            ORR     R6, R3			//HEX3
...
DIVIDE:     MOV    R2, #0	
CONT:       CMP    R3, #10		  
            BLT    DIV_END		 //brach if R3<R1
            SUB    R3, #10		
            ADD    R2, #1		
            B      CONT
DIV_END:    MOV	   R1, R2   //quotient in R1 (remainder in R3)   
            MOV    PC, LR
